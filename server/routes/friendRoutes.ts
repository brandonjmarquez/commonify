...
friendRoutes
  .get('/get-friend-playlists/:friendId', async (req: any, res: any) => {
    try {
      ...
      if(encrypted_token) {
        ...
        if(profileRes.status === 401) {
          ...
          const authOptions = {
            ...
            headers: { 'Authorization': 'Basic ' + (new (Buffer as any).from(process.env.client_id + ':' + process.env.client_secret).toString('base64')) },
            json: true
          };
